...
function assertBlocksArray(raw: unknown): asserts raw is Array<Record<string, unknown>> {
...
export function validateSlackBlocksArray(raw: unknown): Array<Block | KnownBlock> {
  assertBlocksArray(raw);
  return raw as unknown as Array<Block | KnownBlock>;
}

export function parseSlackBlocksInput(raw: unknown): Array<Block | KnownBlock> | undefined {
  if (raw == null) {
    return undefined;
  }
